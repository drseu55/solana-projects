...
describe("todo-app", () => {
  ...
  // Set program
  const mainProgram = anchor.workspace.TodoApp;
  ...
  describe('add', () => {
    it('Anyone can add an item to a list', async () => {
      ...
      const result = await addItem({ list, user: adder, name: 'Do something', bounty: 1 * anchor.web3.LAMPORTS_PER_SOL});
      ...
      expect(await getAccountBalance(result.item.publicKey), 'List account balance').equals(1 * anchor.web3.LAMPORTS_PER_SOL);

      let adderNewBalance = await getAccountBalance(adder.key.publicKey);
      expectBalance(adderStartingBalance - adderNewBalance,  anchor.web3.LAMPORTS_PER_SOL, 'Number of lamports removed from adder is equal to bounty');

      const again = await addItem({ list, user: adder, name: 'Another item', bounty: 1 * anchor.web3.LAMPORTS_PER_SOL});
    ...
    it('Fails if the list is full', async () => {
      ...
      expect(adderStartingBalance, 'Adder new balance is unchanged').equals(adderNewBalance);
    });
  ...
  describe('cancel', () => {
    it('List owner can cancel an item', async () => {
      ...
      expectBalance(adderBalanceAfterCancel, adderBalanceAfterAdd + anchor.web3.LAMPORTS_PER_SOL, 'Cancel returns bounty to adder');
    ...
    it('Other users can not cancel an item', async () => {
      ...
      expect(itemBalance, 'Item balance is unchanged after failed cancel').equals(anchor.web3.LAMPORTS_PER_SOL);
    });
