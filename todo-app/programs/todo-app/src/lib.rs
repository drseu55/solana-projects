...
pub mod todo_app {
    ...
    pub fn cancel(ctx: Context<Cancel>, _list_name: String) -> Result<()> {
        ...
        // Return tokens to the item creator
    ...
    pub fn finish(ctx: Context<Finish>, _list_name: String) -> Result<()> {
        ...
        if !is_item_creator && !is_list_owner {
            return Err(error!(TodoListError::FinishPermissions));
        }
...
pub struct NewList<'info> {
    #[account(init, payer=user,
        ...
        seeds=[
            b"todolist", user.to_account_info().key.as_ref(), name_seed(&name)
        ],
        bump)]
...
pub struct Add<'info> {
    ...
    pub list: Account<'info, TodoList>,
    /// CHECK: We don’t want to read the data from the list_owner account.
    /// CHECK: we just need it to assist in the PDA verification.
...
pub struct Cancel<'info> {
    ...
    /// CHECK: We don’t want to read the data from the item_creator account.
    /// CHECK: we just need it to assist in the PDA verification.
...
impl TodoList {
    fn space(name: &str, capacity: u16) -> usize {
        // discriminator + owner pubkey + bump + capacity + name string + vec of item pubkeys
